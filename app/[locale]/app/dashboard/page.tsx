      ...
      {loading ? (
        ...
        <div className="space-y-2.5">
          <div className="flex items-center justify-between">
            ...
            <span className="font-bold az-mono text-sm" style={{ color: "var(--teal)" }}>{fmtBal(azr)}</span>
          </div>
          <div className="flex items-center justify-between">
            <span className="flex items-center gap-1.5">
            ...
            <span className="font-bold az-mono text-sm">{fmtBal(usdt)}</span>
          </div>
        </div>
      )}
...
export default function DashboardPage() {
  ...
  useEffect(() => {
    ...
    setLoading(true);
    Promise.all([
      fetch(`/api/virtual/balance?wallet=${addr}`).then((r) => r.json()),
      fetch(`/api/virtual/stake?wallet=${addr}`).then((r) => r.json()),
      fetch(`/api/referrals?wallet=${addr}`).then((r) => r.json()),
    ]).then(([bal, stakeData, refData]) => {
  ...
  const staked = activeStakes.reduce((s, p) => s + p.amount, 0);
    ...
    <>
      ...
      <div className="p-4 md:p-8 max-w-app">
        <div className="grid grid-cols-1 sm:grid-cols-2 lg:grid-cols-3 xl:grid-cols-5 gap-5 mb-8">
          {isLoading ? (
            ...
            <>
              <KpiCard
                label={t("totalStaked")}
                value={staked.toLocaleString("en-US", { minimumFractionDigits: 2, maximumFractionDigits: 2 })}
                unit="AZR"
              ...
              <KpiCard
                ...
                value={refTotal !== null ? fmtBal(refTotal) : "—"}
                unit="AZR"
                sub="See Referrals page"
              />
              <WalletCard azr={azrBal} usdt={usdtBal} loading={isLoading} />
            </>
          )}
        </div>
        ...
        {isLoading ? (
            ...
            <div className="space-y-4">
              {[0, 1].map((i) => (
                <div key={i} className="grid grid-cols-4 gap-4">
                  <Skeleton className="h-4" /><Skeleton className="h-4" /><Skeleton className="h-4" /><Skeleton className="h-4" />
                </div>
              ))}
            </div>
          </div>
          ...
          <div className="az-card mb-6">
            <h3 className="font-semibold mb-4">{t("activeStakes")} ({activeStakes.length})</h3>
                    ...
                    return (
                      <tr key={pos.id}>
                        ...
                        <td className="py-3 az-mono text-xs" style={{ color: pos.unlocked ? "var(--teal)" : "var(--text-2)" }}>
                          {pos.unlocked ? "Unlocked ✓" : new Date(unlock).toLocaleDateString()}
                        </td>
          ...
          <div className="az-card flex flex-col items-center py-12 text-center mb-6">
        ...
        <div className="az-card mt-2">
          ...
          <div className="flex flex-wrap items-center justify-center gap-8">
            {[
              { name: "BNB Chain", logo: <svg viewBox="0 0 32 32" className="w-7 h-7" fill="none"><circle cx="16" cy="16" r="16" fill="#F0B90B" /><path d="M12 16l4-4 4 4-4 4-4-4zm-4 0l4 4V16l-4-4v4zm12-4v4l4-4-4-4v4zm-4 8l-4-4 4 4 4-4-4 4z" fill="white" opacity="0.9" /></svg> },
              { name: "Tether", logo: <span className="w-7 h-7 rounded-full flex items-center justify-center font-bold text-sm" style={{ background: "#26A17B", color: "white" }}>₮</span> },
              { name: "MetaMask", logo: <svg viewBox="0 0 32 32" className="w-7 h-7" fill="none"><path d="M28 4L17.5 11.8l1.9-4.5L28 4z" fill="#E17726" /><path d="M4 4l10.4 7.9-1.8-4.6L4 4z" fill="#E27625" /><path d="M24 22.4l-2.8 4.3 6 1.7 1.7-5.8-4.9-.2z" fill="#E27625" /><path d="M3.1 22.6l1.7 5.8 6-1.7-2.8-4.3-4.9.2z" fill="#E27625" /><circle cx="16" cy="16" r="5" fill="#E27625" opacity="0.3" /></svg> },
              { name: "Trust Wallet", logo: <svg viewBox="0 0 32 32" className="w-7 h-7" fill="none"><circle cx="16" cy="16" r="16" fill="#3375BB" /><path d="M16 7l8 3.5v6c0 4.4-3.4 8.3-8 9.5-4.6-1.2-8-5.1-8-9.5v-6L16 7z" fill="white" opacity="0.9" /></svg> },
              { name: "WalletConnect", logo: <svg viewBox="0 0 32 32" className="w-7 h-7" fill="none"><circle cx="16" cy="16" r="16" fill="#3B99FC" /><path d="M9.6 13.2c3.5-3.5 9.3-3.5 12.8 0l.4.4-1.5 1.5-.4-.4c-2.5-2.5-6.4-2.5-8.9 0l-.5.4-1.5-1.5.6-.4zm6.4 3.4l2.1 2.1-2.1 2.1-2.1-2.1 2.1-2.1z" fill="white" /></svg> },
            ].map((p) => (
